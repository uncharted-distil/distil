  ...
  beforeMount() {
    ...
    appActions.fetchConfig(this.$store).then(() => {
      ...
      if (appGetters.isTask1(this.$store) && path === HOME_ROUTE) {
        ...
        console.log(
          `Task 1: Routing directly to select target view with dataset=\`${dataset}\``,
          dataset
        );
      ...
      if (
        appGetters.isTask2(this.$store) &&
        (path === HOME_ROUTE || path === SELECT_TARGET_ROUTE)
      ) {
        const dataset = appGetters.getProblemDataset(this.$store);
        let target = appGetters.getProblemTarget(this.$store);
        const taskPromise = datasetActions
          ...
          .then(response => {
            ...
            if (
              ...
              promise = datasetActions
                ...
                .then(() => {
                  ...
                  const ids = variables
                    .filter(v => v.colType === CATEGORICAL_TYPE)
                    .map(v => v.colName);
                  ...
                  const alreadyComposed = variables.find(
                    v => v.colName === idKey
                  );
                  ...
                  if (!alreadyComposed && ids.length > 1) {
                    console.log(
                      `Task 2: Composing ids for grouping`,
                      ids.join(", ")
                    );
                  ...
                  return nextPromise.then(() => {
                    ...
                    const existingGrouping = variables.find(
                      v => v.colName === idKey
                    );
                    const alreadyGrouped =
                      existingGrouping && !!existingGrouping.grouping;
                    ...
                    const xCol = variables
                      .filter(
                        v => v.colName !== target && v.colType === INTEGER_TYPE
                      )
                      .map(v => v.colName)[0];
            ...
            console.log(
              `Task 2: Routing directly to create models view with dataset=\`${dataset}\` and target=\`${target}\``,
              dataset,
              target
            );
            promise.then(() => {
