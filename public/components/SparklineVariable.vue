...
import { TimeseriesExtrema, VariableSummary, Histogram, Bucket } from '../store/dataset/index';
...
export default Vue.extend({
	...
	computed: {
		timeseriesSet(): TimeseriesSet {
			...
			const histogram = this.summary.filtered ? this.summary.filtered : this.summary.baseline;
	...
	methods: {
		...
		mergeExtrema(a: TimeseriesExtrema, b: TimeseriesExtrema): TimeseriesExtrema {
			return {
				x: {
		...
		numericBucketsToTimeseries(key: string, label: string, buckets: Bucket[]): TimeseriesSet {
		...
		categoryBucketsToTimeseries(key: string, label: string, buckets: Dictionary<Bucket[]>): TimeseriesSet {
			...
			// highest sum first
			summaries.sort((a, b) => { return b.sum - a.sum; });
		...
		variableSummaryToTimeseries(key: string, label: string, histogram: Histogram): TimeseriesSet {
			if (histogram.categoryBuckets) {
				return this.categoryBucketsToTimeseries(key, label, histogram.categoryBuckets);
			}
