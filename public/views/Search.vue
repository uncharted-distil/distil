<template>
	<div class="container-fluid d-flex flex-column h-100 search-view">
		...
		<div class="row">
			<file-uploader-status class="file-uploader-status col-12" :status="uploadStatus" :filename="uploadData.filename" :datasetID="uploadData.datasetID" />
		</div>
		<div class="row flex-2 align-items-center justify-content-center">
			<div class="col-12 col-md-6">
				<div class="d-flex">
					<search-bar class="search-search-bar"></search-bar>
					<file-uploader class="file-uploader" @uploadstart="onUploadStart" @uploadfinish="onUploadFinish"></file-uploader>
				</div>
			</div>
		</div>

		<section class="row flex-10">
			<b-tabs align="center" class="search-content" content-class="search-content-tab">
				<b-tab :title="'Models (' + nbSearchModels + ')'" active>
					<model-search-results class="search-search-results" :is-pending="isPending"> </model-search-results>
				</b-tab>

				<b-tab :title="'Datasets (' + nbSearchDatasets + ')'">
					<dataset-search-results class="search-search-results" :is-pending="isPending"> </dataset-search-results>
				</b-tab>
			</b-tabs>
		</section>
	</div>
