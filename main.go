...
func main() {
	...
	registerRoutePost(mux, "/distil/predict/:dataset/:fitted-solution-id", routes.PredictionsHandler(path.Join(config.D3MOutputDir, config.AugmentedSubFolder), pgSolutionStorageCtor, esMetadataStorageCtor, &config, ingestConfig))
