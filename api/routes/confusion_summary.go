...
type ConfusionSummary struct {
	ConfusionSummary *model.Histogram `json:"histogram"`
}

// ResidualsSummaryHandler bins predicted result data for consumption in a downstream summary view.
func ConfusionSummaryHandler(ctor model.PipelineStorageCtor, ctorData model.DataStorageCtor) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		histogram, err := clientData.FetchConfusionSummary(dataset, resultURI, index)
		if err != nil {
			handleError(w, err)
			return
		}
		...
		err = handleJSON(w, ConfusionSummary{
			ConfusionSummary: histogram,
		})
		if err != nil {
			handleError(w, errors.Wrap(err, "unable marshal confusion histogram into JSON"))
			return
		}
	}
}
