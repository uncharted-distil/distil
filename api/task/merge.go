...
func Merge(schemaFile string, dataset string, config *IngestTaskConfig) (string, error) {
	...
	for i, field := range header {
		...
		if v == nil {
			...
			v = model.NewVariable(i, field, field, field, field, model.StringType, model.StringType, "", []string{"attribute"}, []string{model.VarDistilRoleData}, nil, outputMeta.DataResources[0].Variables, false)
		}
		v.Index = i
