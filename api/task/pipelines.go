	...
	for _, v := range mainDR.Variables {
		if v.RefersTo != nil && v.RefersTo["resID"] != nil {
			...
			if res.CanBeFeaturized() || res.ResType == "timeseries" {
				...
				// add the feature variable
				v := model.NewVariable(len(mainDR.Variables), indexName, "group", v.Name, model.StringType, model.StringType, []string{"attribute"}, model.VarRoleMetadata, nil, mainDR.Variables, false)
